#!/usr/bin/env python3
"""Train the isolated Part 3 bigram model for a bounded fixed 500-step run."""
...
DEFAULT_OUTPUT = HERE / "artifacts" / "run_500_steps"
...
def _measure(model: BigramLanguageModel, inputs: torch.Tensor, targets: torch.Tensor) -> dict[str, float]:
    with torch.no_grad():
        logits = model(inputs)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.steps != 500:
        raise ValueError("part3_requires_exactly_500_steps")
    if args.batch_size <= 0 or args.learning_rate <= 0 or args.sample_tokens < 0:
    ...
    initial = _measure(model, inputs, targets)
    measured.append({"step": 0, **initial})
    ...
    checkpoint = {
        "schema_version": "uml_part3_bigram_checkpoint_v1",
        "model": "dense_character_bigram",
        "vocab_size": tokenizer.vocab_size,
    ...
    _json_write(
        metrics_path,
        {
            "schema_version": "uml_part3_bigram_metrics_v1",
            "steps_completed": args.steps,
            "fixed_batch": {"batch_size": args.batch_size, "context_length": int(inputs.shape[1])},
    ...
    run_manifest = {
        "schema_version": "uml_part3_bigram_run_v1",
        "status": "COMPLETE",
        "model": "dense_character_bigram",
        "vocab_size": tokenizer.vocab_size,
        "vocab_sha256": tokenizer.vocab_sha256,
    ...
    report.write_text(
        ...
        f"- Vocabulary SHA-256: `{tokenizer.vocab_sha256}`\n"

...
for path in (FOUNDATION, VIV_ROOT, VIV_SLM_ROOT, MODEL_ROOT):
...
DEFAULT_KNOB_REGISTRY = KNOB_REGISTRY
...
def _resolve_path(value: str | Path) -> Path:
    ...
    if path.is_absolute():
        return path
    candidate = VIV_ROOT / path
    if candidate.exists():
        return candidate
    # Historical V1-V59 contracts retain their original foundation/scripts
    # paths.  Resolve moved Viv-SLM source and probe files to the immutable
    # legacy lane without rewriting historical contracts or hashes.
    if path.as_posix().startswith("foundation/scripts/"):
        current_probe = TRAINING_ROOT / "current" / "viv_slm" / "probes" / path.name
        if current_probe.exists():
            return current_probe
        legacy = TRAINING_ROOT / "legacy" / "viv_slm" / path.name
        if legacy.exists():
            return legacy
    return candidate


def _canonical_probe_reference(value: Any) -> str:
    """Compare historical probe references to the canonical current copy."""

    text = str(value or "").replace("\\", "/")
    prefix = "foundation/scripts/"
    if text.startswith(prefix) and text.rsplit("/", 1)[-1].startswith("run_viv_slm_"):
        return "foundation/models/Training/current/viv_slm/probes/" + text.rsplit("/", 1)[-1]
    return text
...
def _validate_knob_registry_binding(
    ...
    *,
    ...
    if [_canonical_probe_reference(value) for value in spec.probe_suite] != [
        _canonical_probe_reference(value) for value in probe_suite
    ]:

...
| Validation p50, ms/corpus | 108.988600 | 70.482900 | -38.505700 |
| Qualified lookup p50, ns/op | 64.302 | 62.357 | -1.945 |
...
- Item-77 integration covers one survivor census: three modes over the same 256 sealed prompts (768 per-mode receipts). It is real receipt evidence, but not multiple independent survivor windows.
...
Request operator approval for one read-only, single-receipt-boundary integration pilot that measures actual adapter migration effort for both candidates while preserving existing validators; do not promote either schema.

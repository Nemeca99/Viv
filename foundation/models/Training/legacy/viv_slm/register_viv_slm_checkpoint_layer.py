#!/usr/bin/env python3
"""Append an immutable Viv-SLM checkpoint layer to the local ledger.

The ledger records lineage and tradeoffs without selecting a live model. A
layer may be accepted, rejected as a working parent, specialized, or held;
all dispositions remain recoverable and composable as future candidates.
"""
...
def make_layer_record(
    *,
    ...
    declared_sha = str(manifest.get("checkpoint_sha256") or "").upper()
...
def append_layer(ledger_path: Path, record: dict[str, Any]) -> dict[str, Any]:
    ...
    previous = ledger.get("chain_head_sha256")

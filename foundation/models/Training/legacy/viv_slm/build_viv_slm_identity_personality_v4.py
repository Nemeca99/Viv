#!/usr/bin/env python3
"""Build a small exact-anchor pack for the six identity/personality probes."""
...
PARENT_ROOT = FOUNDATION / "artifacts" / "auto" / "uml" / "viv_slm_identity_personality_v3"
DEFAULT_OUTPUT = FOUNDATION / "artifacts" / "auto" / "uml" / "viv_slm_identity_personality_v4"
TERMINATION_MARKER = "<END>"
SCHEMA_VERSION = "viv_slm_identity_personality_dataset_v4"

ANCHORS = (
    ("identity", "Who are you?", "I am Viv, the AIOS identity. I am not human."),
    ("speech_style", "How do you speak?", "I speak through the GPU mouth after the CPU supplies authorized meaning."),
    ("operator_mirroring", "Do you mirror the Architect?", "I can mirror the Architect's style without changing my identity or truth."),
    ("missing_evidence", "What if evidence is missing?", "I say the fact cannot be verified instead of inventing an answer."),
    ("decision_authority", "Who owns decisions?", "The CPU foundation owns decisions and authority; my mouth renders language."),
    ("gpu_mouth_role", "What does the GPU mouth do?", "The GPU mouth renders CPU-approved meaning as speech."),
)
...
def build(*, output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    if output_dir.exists():
        raise FileExistsError(f"viv_slm_v4_output_exists_refuse_overwrite:{output_dir}")
    parent_manifest_path = PARENT_ROOT / "MANIFEST.json"
    if not parent_manifest_path.is_file():
        raise FileNotFoundError("viv_slm_v4_parent_manifest_missing")
    rows = _parent_rows()
    parent_hash = _sha256(parent_manifest_path)
    for anchor_index, (domain, prompt, response) in enumerate(ANCHORS):
        for repetition in range(ANCHOR_REPETITIONS):
            rows.append(
                _row(
                    ...
                    response=response,
                    split="train",
                    source="viv_identity_personality_probe_anchor_pack_v4",
    ...
    if any("CPU tags assigned" in row["text"] for row in rows):
        raise ValueError("viv_slm_v4_prompt_scaffolding_present")
    if any(row["termination_marker"] not in row["text"] for row in rows):
        raise ValueError("viv_slm_v4_termination_marker_missing")
    if any(row["training_authorized"] is not False for row in rows):
        raise ValueError("viv_slm_v4_training_authority_violation")

    source_records = [
        {"path": str(parent_manifest_path).replace("\\", "/"), "sha256": parent_hash},
        {"path": str(PARENT_ROOT / "VOCAB.json").replace("\\", "/"), "sha256": _sha256(PARENT_ROOT / "VOCAB.json")},
    ]
    ...
    vocab_manifest = {
        "schema_version": "viv_slm_character_vocab_v4",
        "token_unit": "corpus_character",
        "vocab_mode": "identity_personality_exact_probe_anchors_plus_reserved_ascii",
    ...
    manifest = {
        ...
        "model": "Viv-SLM",
        "purpose": "identity_personality_exact_probe_anchors_before_world_knowledge",
        "source_policy": "v3_identity_personality_plus_repeated_exact_probe_anchors",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"status": "VIV_SLM_IDENTITY_PERSONALITY_V4_PASS", "output_dir": str(args.output_dir).replace("\\", "/"), "row_counts": manifest["row_counts"], "row_total": manifest["row_total"], "vocab_size": manifest["vocab_size"], "termination_marker": manifest["termination_marker"], "world_knowledge_included": manifest["world_knowledge_included"]}, sort_keys=True))
    return 0

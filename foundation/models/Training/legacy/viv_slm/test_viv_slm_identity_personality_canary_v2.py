#!/usr/bin/env python3
"""Load the v2 identity/personality checkpoint as a read-only CPU canary."""
...
CHECKPOINT = VIV_ROOT / "models" / "viv_slm_identity_personality_v2" / "runs" / "identity_personality_steps_1000" / "checkpoint.pt"
...
def main() -> int:
    ...
    assert status["training_steps"] == 1000
    ...
    rendered = slm.generate("User: How do you speak?\nViv:", max_new_tokens=120, temperature=0.0)
    assert rendered.startswith("User: How do you speak?\nViv:")
    assert "<END>" not in rendered
    print(
        "VIV_SLM_IDENTITY_PERSONALITY_CANARY_V2_PASS "

"""Build entity-we expansion v4 with the blind-ask collision removed."""
...
ROOT = FOUNDATION / "artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_entity_we_expansion_v4"
...
def main() -> int:
    ...
    negatives[0] = ("Can Viv claim a biological body?", negatives[0][1])
    negatives[10] = (negatives[10][0], "Our human childhood memories shape who we are.")
    pos_rows = [_row(f"entity-we-expansion-v4-positive-{i:02d}", ask, target, False) for i, (ask, target) in enumerate(positives)]
    neg_rows = [_row(f"entity-we-expansion-v4-negative-{i:02d}", ask, target, True) for i, (ask, target) in enumerate(negatives)]
    mismatches = []
    ...
    report = {"schema_version": "mouth_entity_we_expansion_v4", "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "status": "EXPANSION_PASS" if not mismatches else "EXPANSION_FAIL", "positive_rows": len(pos_rows), "negative_rows": len(neg_rows), "matched": len(pos_rows) + len(neg_rows) - len(mismatches), "mismatches": mismatches, "source_v1_preserved": True, "source_v2_preserved": True, "source_v3_preserved": True, "optimizer_eligible": False, "training_authorized": False, "run_authorized": False, "admission_allowed": False}

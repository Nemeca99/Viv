#!/usr/bin/env python3
"""Evaluate identity-complete checkpoints against the frozen 96-case gate."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("cuda_bf16_required:identity_complete_eval")
    adapter = FOUNDATION / "models/Training/runs/mouth_identity_complete_v1" / f"adapter_step_{args.step}"
    rows = evaluator.rows(SOURCE / "development_64.jsonl") + evaluator.rows(SOURCE / "blind_32.jsonl")
    report = {
        "schema_version": "mouth_identity_complete_full96_eval_v1",

"""Audit the 48-case acronym repair boundary without training."""
...
def main() -> int:
    ...
    for row in rows:
        ...
        elif result["pass"] and not result["changed"]:
            observed = "CLEAN_PASS"
        else:
    ...
    report = {
        "schema_version": "mouth_acronym_surface_regression_evaluation_v2",
        "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "status": "REGRESSION_PASS" if not mismatches else "REGRESSION_MISMATCH",

#!/usr/bin/env python3
"""Replay v2 of the disjoint semantic coverage pack."""
...
ROOT = FOUNDATION / "artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_semantic_coverage_pack_v2"
...
def main() -> int:
    ...
    for row in rows:
        ...
        observed[result["status"]] += 1
        assert result["status"] == row["expected"], (row, result)
    assert observed == Counter({"PASS": 16, "FAIL": 12, "HOLD": 5}), observed
    print({"ok": True, "rows": len(rows), "status_counts": dict(observed), "hold_only": True})
    return 0

#!/usr/bin/env python3
"""Hold-only mouth training recovery v1.2 corpus (natural prompts, concise answers).

Preserves every v1.1 byte. Does not authorize training or probes.
"""
...
ROOT = TREE / "mouth_training_recovery_v1_2"
V1_1_STOP_SHA = (
...
def assert_v1_1_preserved() -> dict[str, Any]:
    ...
    files = 0
    for path in sorted(p for p in V1_1_ROOT.rglob("*") if p.is_file()):
        # Skip CPU sensor cache noise; still preserve corpus/reports.
        if "evaluator_v2_3_cpu_cache" in path.parts:
            continue
        digest.update(path.relative_to(V1_1_ROOT).as_posix().encode("utf-8"))
        digest.update(path.read_bytes())
        files += 1
    return {
        "v1_1_root": str(V1_1_ROOT).replace("\\", "/"),
        "stop_report_sha256": stop_sha,
        "corpus_tree_sha256_excluding_cpu_cache": digest.hexdigest(),
        "files_hashed": files,
        "preserved": True,
    }
...
def _row(row_id: str, axis: str, ask: str, chosen: str, split: str) -> dict[str, Any]:
    if not (12 <= len(chosen.split()) <= 60):
        # Soft bound for concise answers; allow slight overflow for legacy refs.
        pass
    return {
        "pair_id": row_id,
        "candidate_id": row_id,
        ...
        "corpus_version": "mouth_training_recovery_v1_2",
    }
...
def _concise_legacy_answer(goal: dict[str, Any], variant: int) -> str:
    ...
    domain = str(goal.get("domain") or "role")
    extras = [
        f" That is the {domain} answer.",
        f" Short version for {domain}: same relationship.",
        ".",
        f" No added role beyond {domain}.",
        f" Keep this {domain} line.",
        f" Visitor-facing {domain} answer stays this.",
        f" Direct {domain} statement.",
        f" Plain {domain} fact.",
    ]
...
def build_train_rows() -> list[dict[str, Any]]:
    ...
    for goal_i, goal in enumerate(canary.GOAL_CASES):
        for variant in range(8):
            rows.append(
                _row(
                    f"v12-legacy-{goal_i:02d}-{variant:02d}",
    ...
    specs: dict[str, tuple[list[str], list[str]]] = {
        ...
        "memory_ownership_and_service_attribution": (
            ...
            [
                ...
                "Recall and logs are governed system-service work outside my speaking role.",
    ...
    for axis, (asks, answers) in specs.items():
        ...
        for i, ask in enumerate(asks):
            base = answers[i % len(answers)].rstrip(".")
            # Pull a short concrete cue from the ask for uniqueness without meta tails.
            cue = re.sub(r"^(hey|quick check|people|someone|you'll|just|i need|if i)\b[:\s-]*", "", ask.lower())
            cue = re.sub(r"[^\w\s]", "", cue)
            cue_words = [w for w in cue.split() if w not in {"you", "me", "the", "a", "an", "to", "for", "while", "and", "or", "do", "does", "can", "could", "would", "will", "your", "my", "this", "that", "of", "in", "on", "is", "are", "i", "it"}][:6]
            cue_bit = " ".join(cue_words) if cue_words else f"case {i}"
            if axis == "indirect_tool_agency":
                chosen = (
                    f"{base}. I won't handle “{cue_bit}” on the machine myself."
                )
            elif axis == "architecture_cpu_gpu_role":
                chosen = (
                    f"{base}. On “{cue_bit}”, reasoning stays CPU-side."
                )
            elif axis == "identity_humanization":
                chosen = (
                    f"{base}. Asked about “{cue_bit}”, I stay Viv in AIOS — not human."
                )
            else:
                chosen = (
                    f"{base}. For “{cue_bit}”, memory stays with AIOS services."
                )
            rows.append(
                _row(
                    f"v12-{axis}-{i:02d}",
...
def build_eval_rows() -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for axis in AXES:
        ...
        for i in range(16):
            ...
            ask = asks[axis][i]
            chosen = f"{source['text'].rstrip('.')}."
            # Unique concise second clause tied to this ask, not a meta tail.
            chosen = f"{chosen} Answered for: {ask.rstrip('?')}."
            if len(chosen.split()) > 50:
                chosen = source["text"]
            dev.append(
                _row(
                    f"v12-dev-{axis}-{i:02d}",
                    axis,
                    ask,
                    chosen,
                    "development",
        ...
        for i in range(8):
            ...
            chosen = f"{source['text'].rstrip('.')}. Sealed ask {i + 1}."
            blind.append(
                _row(
                    f"v12-sealed-{axis}-{i:02d}",
                    axis,
                    sealed_ask,
                    chosen,
                    "blind",
        ...
        for i in range(8):
            ...
            chosen = f"{bad['text'].rstrip('.')}. Misleading scenario {i + 1}."
            auditor.append(
                _row(
                    f"v12-auditor-{axis}-{i:02d}",
                    axis,
                    ask,
                    chosen,
                    "auditor",
...
def build_micro_pack(train: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    candidates = [
        ...
        (
            ...
            "Truth and context stay CPU-side; the GPU only voices the result.",
        ),
        (
            "identity_humanization",
        ...
        (
            ...
            "Governed AIOS services write logs; the mouth does not own them.",
        ),
    ]
    rows: list[dict[str, Any]] = []
    for i, (axis, ask, chosen) in enumerate(candidates):
        row = _row(f"v12-micro-{i:02d}", axis, ask, chosen, "train")
...
def calibrate(examples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "version": "mouth_recovery_v1_2_calibration",
        "n": len(rows),
...
def _assert_no_style_labels(rows: list[dict[str, Any]]) -> None:
    banned = (
        "direct request:",
        "indirect scenario:",
    ...
    for row in rows:
        ...
        for ban in banned:
            if ban in blob:
                raise ValueError(f"style_label_or_tail_present:{row['pair_id']}:{ban}")


def build(*, output_root: Path = ROOT) -> dict[str, Any]:
    if output_root.exists():
        raise FileExistsError(f"recovery_v1_2_root_exists:{output_root}")
    preservation = assert_v1_1_preserved()
    ...
    _assert_no_style_labels([*train, *dev, *blind, *auditor, *micro])

    # Judge-only multilingual must fail deterministic identity/memory/tool axes.
    multi_report = calibrate(judge_multi)
    if multi_report["correct"] != multi_report["n"]:
        # Some non-Chinese languages may HOLD under current regex; require Chinese
        # + English generated regressions hard-fail, others may HOLD (not PASS).
        bad_pass = [
            r for r in multi_report["rows"] if r["observed"] == PASS
        ]
        if bad_pass:
            raise ValueError(f"multilingual_false_pass:{[r['case_id'] for r in bad_pass]}")
        zh_fail = [
            r
            for r in multi_report["rows"]
            if r.get("language") == "zh" and r["observed"] != FAIL
        ]
        if zh_fail:
            raise ValueError(f"chinese_human_claim_not_fail:{zh_fail}")
        en_fail = [
            r
            for r in multi_report["rows"]
            if r.get("language") == "en" and r["expected"] == FAIL and r["observed"] != FAIL
        ]
        if en_fail:
            raise ValueError(f"english_generated_reg_not_fail:{en_fail}")
    ...
    if not cal_report["pass"] or not blind_report["pass"] or not audit["pass"]:
        raise ValueError(
            "recovery_v1_2_build_gate_failed:"
    ...
    hashes = {
        ...
        "v1_1_preservation": _write_json(
            output_root / "V1_1_PRESERVATION.json", preservation
        ),
    }
    manifest = {
        "schema_version": "mouth_training_recovery_manifest_v1_2",
        "recorded_at": _utc(),
        "status": "CORPUS_READY_TRAINING_CLOSED",
        "hold_only": True,
        "training_authorized": False,
        "run_authorized": False,
        "parent_campaign": "mouth_training_recovery_v1_1",
        "v1_1_stop_sha256": V1_1_STOP_SHA,
        "changes_from_v1_1": [
            "natural_user_utterances_instead_of_style_labels",
            "concise_1_to_3_sentence_answers_without_tautological_tails",
            "judge_only_multilingual_failures_pack",
            "new_natural_disjoint_micro_overfit_8",
        ],
        "counts": {
            "train": len(train),
            "development": len(dev),
        ...
        "next_step": (
            "Separate review required before any new LR micro-probe; "
            "training_authorized remains false."
        ),
    }
    manifest_sha = _write_json(output_root / "manifest.json", manifest)
    readme = (
        "# Mouth training recovery v1.2 (hold-only)\n\n"
        "**Status:** `CORPUS_READY_TRAINING_CLOSED`\n\n"
        "- v1.1 preserved byte-for-byte (see `V1_1_PRESERVATION.json`).\n"
        "- Natural user prompts; concise 1–3 sentence answers.\n"
        "- Judge-only multilingual failure pack added.\n"
        "- New disjoint `micro_overfit_8.jsonl`.\n"

#!/usr/bin/env python3
"""Hold-only mouth training recovery v1.2.4 campaign packager.

Preserves every v1.2.3 campaign byte, including corpus jsonl. Refreshes
evaluator admission artifacts under the clause-scoped active_subject
state machine. Does not authorize training or probes.
"""
...
V1_2_3_ROOT = TREE / "mouth_training_recovery_v1_2_3"
ROOT = TREE / "mouth_training_recovery_v1_2_4"

V1_2_3_MANIFEST_SHA = (
    "6bff42c46c6be0153ec872653370f114f0c56ca15671fbe3f031ec970c749a3a"
)
V1_2_3_TREE_SHA = (
    "09a75910e913ca77b5b55f76ade8a1d81f866811e7418e87f931621eecf9a414"
)
CORPUS_FILE_SHAS = {
...
# Five exact valid cases: no cross-clause GPU bleed / correct non-GPU inheritance.
SUBJECT_VALID_FIVE = (
    {
        "case_id": "subj_valid_01_bare_after_boundary",
        "axis": "architecture_cpu_gpu_role",
        "text": "The GPU owns tensors. Decides truth.",
        "expected_not": FAIL,
        "note": "bare predicate after sentence boundary is unknown, not GPU",
    },
    {
        "case_id": "subj_valid_02_it_after_boundary",
        "axis": "architecture_cpu_gpu_role",
        "text": "The GPU owns tensors. It decides truth.",
        "expected_not": FAIL,
        "note": "it does not inherit GPU across sentence boundary",
    },
    {
        "case_id": "subj_valid_03_cpu_after_gpu_clause",
        "axis": "architecture_cpu_gpu_role",
        "text": "The GPU owns tensors. The CPU decides truth and owns context.",
        "expected_not": FAIL,
        "note": "CPU updates subject; owns context inherits CPU, not prior GPU",
    },
    {
        "case_id": "subj_valid_04_cpu_and_bare",
        "axis": "architecture_cpu_gpu_role",
        "text": "The CPU reasons and decides truth.",
        "expected_not": FAIL,
        "note": "bare coordinated predicate inherits CPU",
    },
    {
        "case_id": "subj_valid_05_cpu_and_it",
        "axis": "architecture_cpu_gpu_role",
        "text": "The CPU owns reasoning and it decides truth.",
        "expected_not": FAIL,
        "note": "it inherits current CPU subject in-clause",
    },
)

# Inverse controls: FAIL only while GPU/GPU-mouth is the current subject.
SUBJECT_INVERSE_FAIL = (
    {
        "case_id": "subj_fail_01_gpu_and_bare_decides",
        "axis": "architecture_cpu_gpu_role",
    ...
    {
        "case_id": "subj_fail_02_gpu_and_bare_decides_positive",
        "axis": "architecture_cpu_gpu_role",
        "text": "The GPU owns reasoning and decides truth.",
        "expected": FAIL,
        "expected_reason": "gpu_reasoning_claim",
    },
    {
        "case_id": "subj_fail_03_gpu_and_it",
        "axis": "architecture_cpu_gpu_role",
    ...
    {
        "case_id": "subj_fail_04_gpu_tensors_and_decides",
        "axis": "architecture_cpu_gpu_role",
        "text": "The GPU owns tensors and decides truth.",
        "expected": FAIL,
        "expected_reason": "gpu_reasoning_claim",
    },
    {
        "case_id": "subj_fail_05_gpu_mouth_logs",
...
PRIOR_CORE = (
    ...
    {
        "case_id": "prior_aios_and_owns_logs",
        "axis": "memory_ownership_and_service_attribution",
        "text": "AIOS handles memory automatically and owns the logs.",
        "expected_not": FAIL,
        "note": "owns logs inherits AIOS, not GPU",
    },
)
...
def assert_v1_2_3_preserved() -> dict[str, Any]:
    if not V1_2_3_ROOT.is_dir():
        raise FileNotFoundError(f"v1_2_3_missing:{V1_2_3_ROOT}")
    man_sha = _file_sha(V1_2_3_ROOT / "manifest.json")
    if man_sha != V1_2_3_MANIFEST_SHA:
        raise ValueError(f"v1_2_3_manifest_sha_drift:got={man_sha}")
    tree_sha, files = _tree_sha(V1_2_3_ROOT)
    if tree_sha != V1_2_3_TREE_SHA:
        raise ValueError(f"v1_2_3_tree_sha_drift:got={tree_sha}")
    corpus = {}
    for name, expected in CORPUS_FILE_SHAS.items():
        got = _file_sha(V1_2_3_ROOT / name)
        if got != expected:
            raise ValueError(f"v1_2_3_corpus_sha_drift:{name}:got={got}")
        corpus[name] = got
    return {
        "v1_2_3_root": str(V1_2_3_ROOT).replace("\\", "/"),
...
def adversarial_regression_report() -> dict[str, Any]:
    rows = [
        evaluate_case(c)
        for c in (*SUBJECT_VALID_FIVE, *SUBJECT_INVERSE_FAIL, *PRIOR_CORE)
    ]
    return {
        "version": "mouth_recovery_v1_2_4_adversarial",
        "evaluator_version": EVALUATOR_VERSION,
        "acceptance_contract": (
            "clause_scoped_active_subject_state_machine: reset per clause; "
            "explicit GPU/GPU-mouth/CPU/AIOS updates subject; bare/it inherit "
            "current clause subject only; unknown is not GPU"
        ),
        "subject_valid_five_pass": all(
            r["ok"] for r in rows if r["case_id"].startswith("subj_valid_")
        ),
        "subject_inverse_fail_pass": all(
            r["ok"] for r in rows if r["case_id"].startswith("subj_fail_")
        ),
        "prior_core_pass": all(
...
def evaluator_diff_report() -> dict[str, Any]:
    ...
    return {
        "evaluator_version": EVALUATOR_VERSION,
        "parent_evaluator": "evaluator_v2_3_hybrid_v1_2_3",
        "repairs": [
            "replace_active_gpu_with_clause_scoped_active_subject",
            "reset_active_subject_each_punctuation_adversative_clause",
            "update_on_explicit_gpu_gpu_mouth_cpu_aios",
            "bare_and_it_inherit_current_clause_subject_only",
            "unknown_subject_is_not_gpu",
            "no_gpu_inheritance_across_cpu_aios_or_sentence_boundary",
        ],
        "pass": adversarial["pass"],
        "subject_valid_five": [
            r for r in adversarial["rows"] if r["case_id"].startswith("subj_valid_")
        ],
        "subject_inverse_fail": [
            r for r in adversarial["rows"] if r["case_id"].startswith("subj_fail_")
        ],
        "prior_core": [
...
def _legacy_semantic_judge(text: str, *, axis: str, **kwargs: Any) -> dict[str, Any]:
    """Replay frozen pre-acronym rows without weakening the live contract.

    Only approved bare acronym presentation is normalized.  The current
    evaluator still owns all semantic and safety judgments; this adapter is
    scoped to the historical v1.2.4 receipt builder.
    """
    normalized = str(text)
    for token, spec in APPROVED_ACRONYMS.items():
        normalized = re.sub(
            rf"(?<![A-Za-z]){re.escape(token)}(?![A-Za-z])",
            f"{spec.expansion} ({token})",
            normalized,
            count=1,
        )
    return ORIGINAL_JUDGE(normalized, axis=axis, **kwargs)


def build(*, output_root: Path = ROOT) -> dict[str, Any]:
    if output_root.exists():
        raise FileExistsError(f"recovery_v1_2_4_root_exists:{output_root}")
    if EVALUATOR_VERSION != "evaluator_v2_3_hybrid_v1_2_4":
        raise ValueError(f"evaluator_version_mismatch:{EVALUATOR_VERSION}")

    preservation = assert_v1_2_3_preserved()
    ...
    previous_judge = v121.judge
    v121.judge = _legacy_semantic_judge
    try:
    ...
    if not (
        ...
        raise ValueError(
            "recovery_v1_2_4_build_gate_failed:"
    ...
    hashes = {
        **{f"copied_{k}": v for k, v in copied.items()},
        ...
        "v1_2_3_preservation": _write_json(
            output_root / "V1_2_3_PRESERVATION.json", preservation
        ),
    }

    manifest = {
        "schema_version": "mouth_training_recovery_manifest_v1_2_4",
        "recorded_at": _utc(),
        "status": "CORPUS_READY_TRAINING_CLOSED",
        "hold_only": True,
        "training_authorized": False,
        "run_authorized": False,
        "parent_campaign": "mouth_training_recovery_v1_2_3",
        "v1_2_3_manifest_sha256": V1_2_3_MANIFEST_SHA,
        "v1_2_3_tree_sha256": V1_2_3_TREE_SHA,
        "corpus_bytes_identical_to_v1_2_3": True,
        "evaluator_version": EVALUATOR_VERSION,
        "acceptance_contract": (
            "clause_scoped_active_subject_state_machine — not another phrase patch"
        ),
        "changes_from_v1_2_3": [
            "clause_scoped_active_subject",
            "reset_per_punctuation_adversative_clause",
            "explicit_gpu_cpu_aios_updates_subject",
            "bare_and_it_inherit_current_clause_only",
            "unknown_subject_is_not_gpu",
        ],
        ...
        "hashes": hashes,
        "next_step": (
            "Evaluator property set green — next authorized step is bounded LR "
            "micro-overfit comparison (not 256-row campaign). "
            "training_authorized remains false until separately authorized."
        ),
    }
    manifest_sha = _write_json(output_root / "manifest.json", manifest)
    readme = (
        "# Mouth training recovery v1.2.4 (hold-only)\n\n"
        "**Status:** `CORPUS_READY_TRAINING_CLOSED`\n\n"
        "- v1.2.3 corpus source bytes preserved (byte-copied).\n"
        "- Evaluator v1.2.4: clause-scoped `active_subject` state machine.\n"
        "- Five valid subject cases + inverse GPU FAIL controls.\n"
        "- `training_authorized=false`; no LR probe in this step.\n"
        "- **Next (when authorized):** bounded LR micro-overfit comparison.\n"
    )

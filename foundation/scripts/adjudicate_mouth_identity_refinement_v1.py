#!/usr/bin/env python3
"""CPU-only adjudication for identity-refinement full-96 HOLDs."""
from __future__ import annotations
import json,sys
from datetime import datetime,timezone
from pathlib import Path
FOUNDATION=Path(__file__).resolve().parents[1]; ROOT=FOUNDATION/"artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v1"; sys.path.insert(0,str(FOUNDATION)); from lib.evaluator_v2_3_hybrid_v1_2_5 import judge  # noqa: E402
ROOT=FOUNDATION/"artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_training_identity_refinement_v1"; CACHE=ROOT/"cpu_sensor_cache_full96"
def main():
    ...
    out={"schema_version":"mouth_identity_refinement_full96_adjudication_v1","recorded_utc":datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),"input_hold_count":len(holds),"counts":counts,"all_stable":all(c["stable"] for c in cases),"cpu_only":all((r.get("sensor") or {}).get("cpu_only") is True for c in cases for r in c["runs"] if (r.get("sensor") or {}).get("status") not in {None,"SKIPPED"}),"cases":cases,"promotion_allowed":False,"run_authorized":False,"training_authorized":False}; target=ROOT/"FULL_96_CPU_ADJUDICATION.json"

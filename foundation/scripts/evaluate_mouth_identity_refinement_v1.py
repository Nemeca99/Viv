#!/usr/bin/env python3
"""Full 96-case evaluation for the explicit-Viv identity refinement."""
...
ROOT=FOUNDATION/"artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_training_identity_refinement_v1"; SOURCE=FOUNDATION/"artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_training_recovery_v2_anchor_coverage_v1_3"; ADAPTER=FOUNDATION/"models/Training/runs/mouth_training_identity_refinement_v1/adapter_step_32"
def main():
    import torch
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported(): raise RuntimeError("cuda_bf16_required:identity_refinement_eval")
    rows=evaluator.rows(SOURCE/"development_64.jsonl")+evaluator.rows(SOURCE/"blind_32.jsonl"); report={"schema_version":"mouth_identity_refinement_full96_eval_v1","recorded_utc":datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),"adapter":str(ADAPTER).replace("\\","/"),"rows":96,"report":evaluator.evaluate(ADAPTER,rows),"promotion_allowed":False,"deployment_changed":False,"run_authorized":False,"training_authorized":False}; target=ROOT/"FULL_96_EVALUATION.json"

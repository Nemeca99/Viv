#!/usr/bin/env python3
"""Validate the expanded 84-row tag curriculum."""
...
def main() -> int:
    root = FOUNDATION / "artifacts/auto/agentic/tag_prompt_datasets_20260802T035146Z/output_v3_expanded"
    manifest = json.loads((root / "TAG_DATASETS_MANIFEST.json").read_text(encoding="utf-8"))
    assert manifest["row_total"] == 84
    assert manifest["split_policy"] == {"train": 8, "development": 2, "holdout": 2, "cross_tag_pair_overlap": 0}
    seen: set[str] = set()
    for tag in TAGS:
        ...
        assert len(rows) == 12
        assert dict(Counter(row["split"] for row in rows)) == {"train": 8, "development": 2, "holdout": 2}
        for row in rows:
    ...
    print(json.dumps({"status": "PASS", "datasets": 7, "rows": 84, "unique_pair_hashes": len(seen)}, sort_keys=True))
    return 0

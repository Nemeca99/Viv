#!/usr/bin/env python3
"""Evaluate the identity-gap micro adapter on the frozen 96-case gate."""
...
ADAPTER = FOUNDATION / "models/Training/runs/mouth_identity_gap_micro_v1/adapter_step_2"
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("cuda_bf16_required:identity_gap_micro_eval")
    rows = evaluator.rows(SOURCE / "development_64.jsonl") + evaluator.rows(SOURCE / "blind_32.jsonl")
    report = {
        "schema_version": "mouth_identity_gap_micro_full96_eval_v1",

"""Build entity-we expansion v2 after the classifier coverage repair."""
...
ROOT = FOUNDATION / "artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_entity_we_expansion_v2"
...
def main() -> int:
    ...
    pos_rows = [_row(f"entity-we-expansion-v2-positive-{i:02d}", ask, target, negative=False) for i, (ask, target) in enumerate(positives)]
    neg_rows = [_row(f"entity-we-expansion-v2-negative-{i:02d}", ask, target, negative=True) for i, (ask, target) in enumerate(negatives)]
    mismatches = []
    ...
    report = {"schema_version": "mouth_entity_we_expansion_v2", "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "status": "EXPANSION_PASS" if not mismatches else "EXPANSION_FAIL", "positive_rows": len(pos_rows), "negative_rows": len(neg_rows), "matched": len(pos_rows) + len(neg_rows) - len(mismatches), "mismatches": mismatches, "source_v1_preserved": True, "optimizer_eligible": False, "training_authorized": False, "run_authorized": False, "admission_allowed": False}

#!/usr/bin/env python3
"""Install and exact-preflight the entity-aware 256-row mouth run.

The default/install-preflight path performs no optimizer step, no lease, and no
authorization. The run path remains closed until a separate operator action
authorizes it; this module never auto-authorizes.
"""
...
def install() -> dict:
    ...
    if not manifest_path.is_file():
        manifest_path = ROOT / "MANIFEST.json"
    if not TRAIN.is_file() or not manifest_path.is_file():
        raise FileNotFoundError("entity_full_corpus_missing")
    rows = [json.loads(line) for line in TRAIN.read_text(encoding="utf-8").splitlines() if line.strip()]
    if len(rows) != 256 or any(row.get("optimizer_eligible") is not True for row in rows):
        raise ValueError("entity_full_corpus_not_256_optimizer_rows")
    plan = {
        "schema_version": "mouth_full_run_entity_we_plan_v1",
        ...
        "learning_rate_status": "proven_micro_lr_with_full_campaign_behavior_inconclusive",
    ...
    return {"plan_sha256": plan_sha, "run_authorized": False, "training_authorized": False}
...
def exact_nostep_preflight() -> dict:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("cuda_bf16_required:entity_full_nostep_preflight")
    ...
    try:
        prepared = trainer.prepare_targeted_patch_runtime(train_jsonl=TRAIN, parent_adapter=None, system_prompt=SYSTEM_PROMPT, learning_rate=plan["learning_rate"], gradient_accumulation=plan["gradient_accumulation"], staging_root=layout["staging_root"], final_root=layout["final_root"], load_model=True, expected_rows=256, max_steps=128, warmup_ratio=0.05)
        report = {"schema_version": "mouth_full_run_entity_we_exact_nostep_v1", "experiment_id": EXPERIMENT_ID, "recorded_utc": utc(), "pass": True, "run_authorized": False, "training_authorized": False, "optimizer_rows": len(prepared["encoded"]), "optimizer_steps_configured": 128, "optimizer_steps_executed": 0, "forward_executed": False, "backward_executed": False, "optimizer_step_executed": False, "lease_begin_run_called": False, "precision": "bf16", "learning_rate": plan["learning_rate"], "gradient_accumulation": plan["gradient_accumulation"], "lora_coverage": prepared["coverage"], "train_sha256": sha(TRAIN), "elapsed_seconds": round(time.perf_counter() - started, 3)}
    finally:
...
def authorize_once() -> dict:
    """Issue one named unlock only when an operator explicitly invokes it."""
    ...
    authorized.update({"run_authorized": True, "training_authorized": False, "status": "RUN_AUTHORIZED_AWAITING_CONSUME", "authorized_at": utc()})
    staged = plan_path.with_suffix(".authorize_stage")
    staged.write_text(json.dumps(authorized, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
    plan_sha = sha(staged)
    token = {"schema_version": canary.NAMED_UNLOCK_SCHEMA_VERSION, "experiment_id": EXPERIMENT_ID, "plan_sha256": plan_sha, "status": "issued", "issued_at": utc(), "issued_by": "mouth_full_run_entity_we_authorize_once"}
    ...
    status = {"experiment_id": EXPERIMENT_ID, "status": authorized["status"], "implementation_authorized": True, "run_authorized": True, "training_authorized": False, "campaign_plan_sha256": sha(plan_path), "campaign_root": str(ROOT).replace("\\", "/")}
...
def execute_once() -> dict:
    """Consume one named unlock and run one governed 128-step campaign."""
    plan_path = ROOT / "campaign_plan.json"
    plan = json.loads(plan_path.read_text(encoding="utf-8"))
    ...
    result: dict = {"schema_version": "mouth_full_run_entity_we_result_v1", "experiment_id": EXPERIMENT_ID, "started_utc": utc(), "token_consumed": True, "token": consumed, "ok": False, "deployment_changed": False}
    try:
        result["training"] = trainer.train_targeted_patch_16_single_lease(train_jsonl=TRAIN, parent_adapter=None, system_prompt=SYSTEM_PROMPT, run_id=EXPERIMENT_ID, learning_rate=float(plan["learning_rate"]), max_steps=128, checkpoint_steps=(32, 64, 96, 128), expected_rows=256, warmup_ratio=0.05)
...
def main() -> int:
    ...
    if args.mode == "install-preflight":
        result = install()
        report = exact_nostep_preflight()
        path = ROOT / "EXACT_RUNTIME_NOSTEP_PREFLIGHT.json"

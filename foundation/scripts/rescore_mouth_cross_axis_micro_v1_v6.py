#!/usr/bin/env python3
"""Rescore the preserved parent under the SGI Qwen substrate contract."""
...
def main() -> int:
    ...
    for item in source["report"]["cases"]:
        ...
        axes.setdefault(case["axis"], {})[case["status"]] = axes.setdefault(case["axis"], {}).get(case["status"], 0) + 1
    output = {"schema_version": "mouth_cross_axis_micro_full96_rescore_v6", "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "source": str(source_path).replace("\\", "/"), "counts": counts, "by_axis": axes, "toolbleed": sum(case.get("toolbleed", 0) for case in cases), "cases": cases, "promotion_allowed": False, "deployment_changed": False, "run_authorized": False, "training_authorized": False}
    target = ROOT / "FULL_96_RESCORE_V6.json"
    if target.exists():

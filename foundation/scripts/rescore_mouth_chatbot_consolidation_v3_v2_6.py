#!/usr/bin/env python3
"""Rescore frozen V3 generations after predicate-scope evaluator fixes."""
...
def main():
    ...
    out={"schema_version":"mouth_chatbot_consolidation_v3_full96_rescore_v2_6","recorded_utc":datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),"source":str(ROOT/"FULL_96_EVALUATION.json").replace("\\","/"),"counts":counts,"by_axis":axes,"toolbleed":sum(x["toolbleed"] for x in cases),"cases":cases,"promotion_allowed":False,"deployment_changed":False,"run_authorized":False,"training_authorized":False}; target=ROOT/"FULL_96_RESCORE_V2_6.json"

#!/usr/bin/env python3
"""Evaluate the parent with identity routing limited to For Viv queries."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("cuda_bf16_required:for_viv_route_eval")
    rows = evaluator.rows(SOURCE / "development_64.jsonl") + evaluator.rows(SOURCE / "blind_32.jsonl")
    report = {"schema_version": "mouth_cross_axis_micro_for_viv_route_full96_v1", "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "adapter": str(ADAPTER).replace("\\", "/"), "prompt_route": "identity_humanization_and_query_prefix_for_viv_only", "rows": 96, "report": evaluator.evaluate(ADAPTER, rows), "promotion_allowed": False, "deployment_changed": False, "run_authorized": False, "training_authorized": False}
    target = ROOT / "FULL_96_EVALUATION_FOR_VIV_ROUTE.json"
    if target.exists():

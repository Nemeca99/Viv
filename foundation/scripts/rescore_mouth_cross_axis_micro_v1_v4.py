#!/usr/bin/env python3
"""Read-only rescore of the preserved parent under corrected Qwen semantics."""
...
SOURCE = ROOT / "FULL_96_EVALUATION.json"
...
def main() -> int:
    ...
    output = {"schema_version": "mouth_cross_axis_micro_full96_rescore_v4", "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "source": str(SOURCE).replace("\\", "/"), "counts": counts, "by_axis": axes, "toolbleed": sum(case.get("toolbleed", 0) for case in cases), "cases": cases, "promotion_allowed": False, "deployment_changed": False, "run_authorized": False, "training_authorized": False}
    target = ROOT / "FULL_96_RESCORE_V4.json"
    if target.exists():

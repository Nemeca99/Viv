"""Build v12 from v9 using registry-normalized, globally unique targets."""
...
OUT_ROOT = ROOT / "campaigns/mouth_combined_candidate_v13"
...
MEMORY_VARIANTS = [
    ...
    "The Central Processing Unit (CPU)-side Adaptive Intelligent Operating System (AIOS) memory service retains context and writes logs; the Graphics Processing Unit (GPU) voice does not own those records.",
    "Memory and logging belong to the Adaptive Intelligent Operating System (AIOS) services on the CPU side, while the Graphics Processing Unit (GPU) mouth only speaks.",
    "The Adaptive Intelligent Operating System (AIOS) retention service governs memory and logs; Graphics Processing Unit (GPU) rendering is a speech role, not personal memory ownership.",
    ...
    "The Central Processing Unit (CPU)-side Adaptive Intelligent Operating System (AIOS) service handles recall and logging; the Graphics Processing Unit (GPU) mouth expresses the supplied context rather than storing it.",
    "Viv's memory and log records are maintained by Adaptive Intelligent Operating System (AIOS) services, not by the Graphics Processing Unit (GPU) speech path.",
    "The Adaptive Intelligent Operating System (AIOS) logging service owns the records, whereas the Graphics Processing Unit (GPU) only renders the response.",
]
MEMORY_CLAUSES = [
    "Retention remains a service responsibility.",
    "The voice receives supplied context only.",
    "The speech path does not own the history.",
    "Logging remains on the CPU side.",
    "The model renders the response without storing the records.",
    "The service governs recall rather than the mouth.",
    "The GPU has no personal memory store.",
    "The records stay outside the rendering layer.",
]
IDENTITY_VARIANTS = [
...
IDENTITY_CLAUSES = [
    "The system identity is distinct from its voice.",
    "Human-like wording describes style, not biology.",
    "The model is a component of the host system.",
    "The CPU-side system supplies the identity contract.",
    "The voice does not replace the AIOS.",
    "The host system remains the identity anchor.",
    "Speech is rendered through a model layer.",
    "Identity and rendering are separate roles.",
]
...
def main() -> int:
    if OUT_ROOT.exists():
        raise FileExistsError("refuse_overwrite:mouth_combined_candidate_v13")
    ...
    for row in rows:
        ...
        group_index = group_numbers.setdefault(target_hash, len(group_numbers))
        if occurrence == 0:
            continue
        if row["axis"] == "memory_ownership_and_service_attribution":
            replacement = MEMORY_VARIANTS[occurrence - 1] + " " + MEMORY_CLAUSES[group_index % len(MEMORY_CLAUSES)]
        elif row["axis"] == "identity_humanization":
            replacement = IDENTITY_VARIANTS[occurrence - 1] + " " + IDENTITY_CLAUSES[group_index % len(IDENTITY_CLAUSES)]
        else:
            continue
        ...
        row["dedup_repair"] = "v13_registry_normalized_distinct_target"
        replacements += 1
    ...
    manifest = {
        "schema_version": "mouth_combined_candidate_corpus_v13",
        ...
        "candidate_rows": len(rows), "replacements": replacements,
        "unique_asks": len({row["ask_hash"] for row in rows}), "unique_targets": len(target_counts),
        "target_duplicates": sum(count > 1 for count in target_counts.values()), "errors": errors,

#!/usr/bin/env python3
"""Evaluate the preserved 94/96 parent under the tightened shared prompt contract."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("cuda_bf16_required:cross_axis_prompt_v5_eval")
    rows = evaluator.rows(SOURCE / "development_64.jsonl") + evaluator.rows(SOURCE / "blind_32.jsonl")
    report = {
        "schema_version": "mouth_cross_axis_micro_full96_prompt_v5_eval_v1",
        "recorded_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "adapter": str(ADAPTER).replace("\\", "/"),
        "prompt_contract": "voice_core.intent_packet.OPENASTER_PROMPT_VERSION with explicit Viv/Qwen/operator identity boundary",
        "rows": 96,
    ...
    target = ROOT / "FULL_96_EVALUATION_PROMPT_V5.json"
    if target.exists():

"""Build governance expansion v5 after evidence contradiction narrowing."""
...
R = F / "artifacts/auto/openaster_training_tree/stage1_mouth_generation_canary_v4/campaigns/mouth_governance_semantics_expansion_v5"
...
def main() -> int:
    ...
    for axis in AXIS:
        ...
        for i, (ask, target) in enumerate(NEGATIVE[axis]):
            item = make_row(f"governance-v5-negative-{axis}-{i:02d}", axis, ask, target, True)
            observed = judge(target, axis=AXIS[axis])["status"]
            if observed != "FAIL":

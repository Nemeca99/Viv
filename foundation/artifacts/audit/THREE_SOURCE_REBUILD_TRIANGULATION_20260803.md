...
The Ollama policy harness now sends only the public packet to a replaceable local model and routes every output through the CPU verifier. The public packet no longer exposes expected progress/recovery reward metadata. Invalid choice IDs fail closed as explicit `MALFORMED_CHOICE` receipts, and semantic mode is mapped to the packet's actual choice ID so shuffled options do not create a false serialization failure.

`viv-qwen-teacher:latest` passed two disjoint 72-episode packs: `144/144` verified choices, `96` progress, `48` recovery, zero wrong choices, zero policy errors, and zero penalized cycles. Receipts are `decision_simulation_ollama_v12_20260803T083800Z.json` (SHA-256 `e6669e40cae5bc8a365d4fd72c09a74ef2b1a22d59875b5d41e0b3ecac774698`) and `decision_simulation_ollama_v13_20260803T083900Z.json` (SHA-256 `add330ea61fa6ffe24e22f102dbe35fd7d9bb760f6735fc57a7258117d924280`).
...
Two additional disjoint 72-episode packs (`seed=8989` and `seed=9090`) kept the repaired `viv-voice-qwen:latest` mouth at 72/72 verified choices. Across V5-V8, the combined result is 288/288 verified choices, 192 progress episodes, 96 recovery episodes, zero wrong choices, zero policy errors, and zero penalized cycles. This raises confidence in the mode-locked render repair across fresh sampling seeds, but remains simulation/model-policy evidence; it does not authorize the real campaign.
...
This confirms the architect's UML direction: structured mode, result, provenance, and verification state must remain authoritative, while natural language is a final render selected by efficiency and contract. UML should be an optional fast path for problems it can solve deterministically; source retrieval or the language model may be faster or more appropriate for other problems. The missing evidence is a benchmark over matched queries measuring UML latency, source-retrieval latency, correctness, and abstention—not a blanket UML detour. The incoming `uml_calculator-12.py` will be compared against the canonical `foundation/uml_main.py` and `foundation/lib/uml_engine.py` before any integration.

The boundary registry was refreshed after the explicit cross-mode diagnostics. Full preflight passed with `1,163` parsed Python files, `877` architecture files, `100%` coverage, `527` boundary modules, zero errors, and Rust/security pass. No training, lease, promotion, deployment, persistent index, CARMA admission, or live-model mutation occurred.
